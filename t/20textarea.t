# $Id: 20textarea.t,v 1.2 2011/10/04 19:58:19 pmh Exp $
...
for(
  ...
  my $out;
  is_xml($out=$empty->textarea($args),$expect_empty,"$name (empty, ref)")
    or diag $out;
  is_xml($out=$empty->textarea(%$args),$expect_empty,"$name (empty, flat)")
    or diag $out;
  is_xml($out=$full->textarea($args),$expect_full,"$name (full, ref)")
    or diag $out;
  is_xml($out=$full->textarea(%$args),$expect_full,"$name (full, flat)")
    or diag $out;
}
